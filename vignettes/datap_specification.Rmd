...
The interpreter parses the datap context, and maps pre-processing steps defined in (1) to actual library functions available in (2). Once your context is loaded into memory, you get - for each tap - an R function that can be called to access a data set.
...
And you can directly navigate to a tap and fetch the data set:
