...
datap is a lightweight DSL (Domain Specific Language) to define configurable, modular, and re-usable data processes. datap configurations can be used to acquire, pre-process, quality-assure, and merge data.

datap is language neutral.

In practice, each datap setup will consist of the following elements:

1. One or more **datap configuration files**. These files define the identifiers of specific data sets, as well as their source and pre-processing steps.
2. One or more **code libraries** in the programming language of your choice. These libraries do the actual units of work of the pre-processing steps defined in (1), like e.g. loading data into memory, data cleaning, merging, etc. The code libraries are typically datap-agnostic.
3. A **datap interpreter**, in the programming language of your choice. The interpreter parses the datap configuration file, and maps pre-processing steps defined in (1) to actual library functions available in (2).
...
* flow control:
	...
	* [`>junction`](#junction): merge multiple joints into one
* data processing:
	* [`>processor`](#processor): unit of work (data acquisition and pre-processing)
	* [`>factory`](#factory): meta-programming construct
* error handling:
	* [`>warning`](#warning)
	* [`>error`](#error)
